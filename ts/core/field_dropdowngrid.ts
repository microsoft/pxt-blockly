...
namespace PxtFields {
    export class FieldDropdownGrid extends Blockly.FieldDropdown {
        ...
        constructor(optionsValues: string[][], columns: number = 1, useTooltips: boolean = true) {
            super(optionsValues);
        ...
        public showEditor_() {
            ...
            const menuItemsDom = menuDom.children;
            let maxWidth = -1;
            for (let i = 0; i < menuItemsDom.length; ++i) {
                const currentSize = goog.style.getSize(menuItemsDom[i]);
