...
namespace pxtblocky {
    ...
    export interface FieldIconDropdownParams {
        columns?: string;
    }

    export class FieldIconDropdown extends Blockly.FieldDropdown {
         ...
         * @constructor
         */
        constructor(menuGenerator: string[][], params: FieldIconDropdownParams) {
            super(menuGenerator);

            if (params) {
                this.columns_ = parseInt(params['columns']);
                this.width_ = parseInt(params['width']);
                this.itemWidth_ = parseInt(params['itemWidth']);
                this.itemHeight_ = parseInt(params['itemHeight']);
                if (!this.itemHeight_) this.itemHeight_ = this.itemWidth_;
            }
            this.addArgType('icondropdown');
        }
        ...
        public showEditor_() {
            ...
            for (var i = 0; i < options.length; i++) {
                ...
                if (!itemWidth) itemWidth = icon.width;
                if (!itemWidth) itemWidth = buttonImg.width;
            }
            if (this.width_) {
...
(Blockly as any).FieldIconDropdown = pxtblocky.FieldIconDropdown;